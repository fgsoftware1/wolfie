...
function build_wolfie
	...
	if not test -f isodir/boot/kernel.elf
		echo "Error: kernel.elf was not moved to isodir/boot/. Exiting."
		exit 1
	end
	grub-mkrescue -o wolfieOS.iso isodir
	if test $status -ne 0
		echo "grub-mkrescue failed. Exiting."
		exit 1
	end
end
